...
async function convertToNrrd(dicomFiles, outputPath) {
    try {
        ...
        // Pass 1: Metadata only
        console.log('Pass 1: Reading metadata for NRRD...');
        for (const filePath of dicomFiles) {
            const dicomFileBuffer = fs.readFileSync(filePath);
            const dicomData = DicomMessage.readFile(dicomFileBuffer.buffer);
            const dataset = DicomMetaDictionary.naturalizeDataset(dicomData.dict);
        ...
        const totalSize = rows * columns * depth;
        console.log(`Allocating NRRD volume data: ${rows}x${columns}x${depth}`);
        const volumeData = new Float32Array(totalSize);
        
        // Pass 2: Pixel Data
        console.log('Pass 2: Reading pixel data for NRRD...');
        for (let z = 0; z < depth; z++) {
            const slice = slices[z];
        ...
        console.error('Error converting DICOM to NRRD:', error);
        throw error;
    }
}
        ...
        async function findJsonFiles(dir) {
            ...
            for (const item of items) {
                ...
                if (item.isDirectory()) {
                    ...
                    // Find associated VTI and NRRD files in the same directory
